function Visualizer(canvasId, sequencer) {
...
// Sequencer events
Visualizer.prototype.onBeat = function(time, step) {
    this.events.push({type: 'beat', time: time, data: step});
}

Visualizer.prototype.onEvent = function(time, eventType, unitId, eventData) {
...
// Beat visuals
Visualizer.prototype.initBeatVisuals = function () {
    this.logoSize = 1.0;
    this.logoSpeed = 1.0;
    this.logoScales = [];
}

Visualizer.prototype.handleBeatEvent = function (event) {
    if (event.data % 4 == 0) {
        this.logoSize = 1.5;
        this.logoSpeed = 1.0;
    }
...
Visualizer.prototype.drawBeatVisuals = function (ctx, timeDiff) {
...
Visualizer.prototype.initNoteVisuals = function() {
    this.unitVisuals = {};
}

Visualizer.prototype.handleNoteEvent = function (event) {
    ...
    if (visuals.type == 'dots') {
        visuals.dots.push({
            ...
            color: dotColors[Math.floor(event.data.pitch / 12)],
            speed: 1.0
        });
...
Visualizer.prototype.drawNoteVisuals = function (ctx, timeDiff) {
...
Visualizer.prototype.drawDotsVisuals = function (ctx, timeDiff, visuals) {
    ctx.save();
...
Visualizer.prototype.drawShapeVisuals = function (ctx, timeDiff, visuals) {
    ctx.save();
...
Visualizer.prototype.lightColorWithOpacity = function (r, g, b, a, light) {
    r *= light;
    g *= light;
    b *= light;
    if (r > 255) r = 255;
